...
define([
    ...
    'bootstrap',
    'moment',
    'gabetest/js/html2canvas_newest/png_download',
    'gabetest/js/FileSaver'

], function($, IPython, dialog, utils, tour, bootstrap, moment, png_download) {

//], function($, IPython, dialog, utils, tour, bootstrap, moment, saveAs) {
    "use strict";
    ...
    var MenuBar = function (selector, options) {
        ...
        //this sets the default value of the function arguments for options, AKA
        // If options returns something, then we use that. If it contains some "falsy" like null we will initialize an empty object
        options = options || {};
        //the following is saying that if base_url exists we use it, otherwise we call the second function
        ...
        //this.getPNG = options.getPNG || getPng
    ...
    MenuBar.prototype._nbconvert = function (format, download) {
        if (format == 'pdf') {
            //the reason why we are doing this is because it is difficult to grab the generated html and convert to pdf
            //therefore we are kind of cheating and using the browser rendered output to print-to-pdf
            console.warn("Since backend PDF rendering is not yet there, this hack is just using the browser's print()")
            var inputToo = confirm("Do you want to print the input boxes too? Gabe-was-here");
            if (!inputToo) {
                $(".input:visible").addClass("show-me-back").hide();
            }
            //this adds a Title for the Name of the Notebook to Print
            $("#site").prepend($("<h1 class='text-center print-artifact'></h1>").text($("#notebook_name").text()));
            //this uses the class we created and removes the empty results from our notebook so that we don't print them
            $(".output_result:empty").parents(".output_wrapper:visible").addClass("show-me-back").hide();
            //this hides the output if we are using the "output hidden" parameter
            $(".output:hidden").parents(".output_wrapper:visible").addClass("show-me-back").hide();
            //this print function will not show everything, we defined hidden elements.
            print()
            //this removes the title we created for our printed page.
            $(".print-artifact").remove();
            //this shows the empty output cells again
            $(".show-me-back").show();

        } else if (format == 'pdf-full') {
            $('div#maintoolbar').click()
            var element = document.documentElement.innerHTML;
            html2pdf(element);

        }else if (format == 'png') {
            //moved the code because we have dependency overlap with html2canvas
            //code shown below is still used with getPng() function
            //this takes a screenshot of the current notebook as a PNG
            ...
            var blob = new Blob([new XMLSerializer().serializeToString(document)], {type: "text/plain;charset=utf-8"});
            saveAs(blob, ($("#notebook_name").text() + ".html"));
    ...
    MenuBar.prototype.bind_events = function () {
        ...
        this.element.find('#download_snb').click(function () {
        ...
        this.element.find('#download_pdf-full').click(function () {
            that._nbconvert('pdf-full', true);
        });

        this.element.find('#download_png').click(function () {
            that._nbconvert('png', true);
        });
        ...
        this.element.find('#download_markdown').click(function () {
            ...
            that._nbconvert('markdown', false);
        });
