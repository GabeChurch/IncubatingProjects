...
class BisectingKMeans private(
  ...
  def run(input: RDD[Vector]): BisectingKMeansModel = {
    ...
    while (activeClusters.nonEmpty && numLeafClustersNeeded > 0 && level < LEVEL_LIMIT) {
      ...
      // GABE Custom added to grab the larger divisible cluster output from the above if statement
      var outInfoA = divisibleClusters.map(x => Seq("d = " + x._1.toString + ", cost =" +x._2.cost.toString + ", size = " + x._2.size.toString).mkString("\n")).mkString("\n")
      logInfo(s"The divisible clusters needed for this iteration were : $outInfoA")
      ...
      if (divisibleClusters.nonEmpty) {
        ...
        for (iter <- 0 until maxIterations) {
          ...
          // Gabe Church Added to track assignments through all Iterations
          var outInfo = newClusters.map(x => Seq("d = " + x._1.toString + ", cost =" +x._2.cost.toString + ", size = " + x._2.size.toString).mkString("\n")).mkString("\n")
          logInfo(s"The assignments for this iteration were : $outInfo")
        }
        ...
        // Custom added by GABE to try and track Indices
        // val InFlat = indices.collect().map(x => x.toString).mkString("\n")
        // val outInfo = assignments.map(x => x._1.toString).collect().mkString("\n")
        // logInfo(s"Indices Gabe saved : $outInfo")

       // var outInfo2 = assignments.map(x => Seq("d = " + x._1.toString + ", norm =" + x._2.norm.toString ).mkString("\n")).collect().mkString("\n")
       // logInfo(s"The Indices saved for this iteration were : $outInfo2")
...
private[clustering] class ClusteringTreeNode private[clustering](
  ...
  /**
    ...
    */
  def toArray: Array[ClusteringTreeNode] = {
    ...
    array.sortWith { case (a, b) =>
      a.index < b.index && a.cost < b.cost && a.size < b.size
    }
  }
    ...
    nodes.foreach { parent =>
      if (parent.children.size > 1) {
        ...
        parent.children.foreach { child =>
          ...
          adjacencyList = adjacencyList :+ ((parentIndex, childIndex, parent.height))
        }
      }
    }
    adjacencyList
  ...
  def toLinkageMatrix: Array[(Int, Int, Double, Int)] = {
    val nodes = toArray.sortWith { case (a, b) => a.height < b.height }
    val leaves = nodes.filter(_.isLeaf)
    val notLeaves = nodes.filterNot(_.isLeaf).filter(_.children.size > 1)
    ...
    def getIndex(map: Map[ClusteringTreeNode, Int], node: ClusteringTreeNode): Int = {
